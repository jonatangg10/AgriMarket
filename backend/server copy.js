  ...
  db.get('SELECT COUNT(*) as count FROM estado', (err, row) => {
    ...
    db.get('SELECT COUNT(*) as count FROM usuarios', (err, row) => {
      ...
      if (row.count === 0) {
        ...
        const stmt = db.prepare(`INSERT INTO usuarios (nombres, apellidos, num_documento, correo, password, rol, fecha_creacion) VALUES (?, ?, ?, ?, ?, ?, ?)`);
      
        usuariosIniciales.forEach(u => {
          stmt.run([u.nombres, u.apellidos, u.num_documento, u.correo, u.password, u.rol, u.fecha_creacion], (err) => {
        ...
        if (err) console.error(err);
          if (row.count === 0) {
            const stmt = db.prepare(`
              ...
              (nombre, usuario_id, estado_id, precio, imagen, stock, etiqueta, categoria) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `);
...
app.get('/api/usuarios', (req, res) => {
  db.all(
    'SELECT id, nombres, apellidos, num_documento, correo, password, rol, fecha_creacion FROM usuarios',
    (err, rows) => {
      if (err) {
